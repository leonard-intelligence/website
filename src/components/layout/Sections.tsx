...
// ============================================================================
// SECTION — Font Specimen (dev preview of all Geist fonts)
// ============================================================================
export function SectionFontSpecimen() {
    const sansWeights = [100, 200, 300, 400, 500, 600, 700, 800, 900];
    const monoWeights = [100, 200, 300, 400, 500, 600, 700, 800, 900];
    const pixelVariants = [
        { name: 'Square', cssVar: '--font-pixel-square' },
        { name: 'Grid', cssVar: '--font-pixel-grid' },
        { name: 'Circle', cssVar: '--font-pixel-circle' },
        { name: 'Triangle', cssVar: '--font-pixel-triangle' },
        { name: 'Line', cssVar: '--font-pixel-line' },
    ];
    // Pixel Line has 7 stylistic sets (ss01-ss06 + ss09) — confirmed via font inspection.
    const pixelLineSets = ['default', 'ss01', 'ss02', 'ss03', 'ss04', 'ss05', 'ss06', 'ss09'];

    return (
        <section
            id="section-font-specimen"
            className="relative"
            style={{ backgroundColor: TOKENS.surface, paddingBlock: '96px', paddingInline: '32px' }}
            aria-label="Font specimen"
        >
            <div className="max-w-[1100px] mx-auto">
                {/* Header */}
                <header style={{ borderBottom: '1px solid rgba(32,32,32,0.15)', paddingBottom: 16, marginBottom: 48 }}>
                    <div className="font-mono" style={{ fontSize: 11, letterSpacing: '0.22em', opacity: 0.6 }}>
                        FONT SPECIMEN · LEONARD INTELLIGENCE
                    </div>
                    <h2 className="font-sans" style={{ fontSize: 36, fontWeight: 500, letterSpacing: '-0.02em', marginTop: 4 }}>
                        Geist family
                    </h2>
                </header>

                {/* ========== GEIST SANS ========== */}
                <SpecimenSection title="Geist Sans" subtitle="Variable · weight 100 → 900">
                    <SpecimenRow label="Display" size={72} fontFamily="var(--font-sans)" weight={500} text="Leonard" />
                    {sansWeights.map((w) => (
                        <SpecimenRow
                            key={`sans-${w}`}
                            label={`Weight ${w}`}
                    ...
                    <SpecimenRow label="400 italic" size={18} fontFamily="var(--font-sans)" weight={400} italic text="The quick brown fox jumps over the lazy dog 0123456789" />
                    <SpecimenRow label="700 italic" size={18} fontFamily="var(--font-sans)" weight={700} italic text="The quick brown fox jumps over the lazy dog 0123456789" />
                </SpecimenSection>

                {/* ========== GEIST MONO ========== */}
                <SpecimenSection title="Geist Mono" subtitle="Variable · weight 100 → 900 · tnum / zero / cv11 features">
                    <SpecimenRow
                        label="Display"
                        size={48}
                        fontFamily="var(--font-mono)"
                        weight={500}
                        text="PROSPECTOR / 24:7 / <2s"
                    />
                    {monoWeights.map((w) => (
                        <SpecimenRow
                            key={`mono-${w}`}
                            ...
                            fontFamily="var(--font-mono)"
                            weight={w}
                            text="EXECUTION 24/7 · SUBAGENTS 03 · LATENCY <2s · ZERO 0Oo"
                        />
                    ))}
                    <SpecimenRow label="400 italic" size={16} fontFamily="var(--font-mono)" weight={400} italic text="EXÉCUTE 1 284 ACTIONS / JOUR EN PRODUCTION" />
                    <SpecimenRow
                        label="With features"
                        size={16}
                        fontFamily="var(--font-mono)"
                        weight={500}
                        text="0123456789 · 0Oo · agent / a"
                        fontFeatures="'tnum' 1, 'zero' 1, 'ss01' 1, 'cv11' 1"
                    />
                </SpecimenSection>

                {/* ========== GEIST PIXEL — 5 variants ========== */}
                <SpecimenSection title="Geist Pixel" subtitle="5 display variants · no weight axis · 7 stylistic sets each">
                    {pixelVariants.map((v) => (
                        <SpecimenRow
                            key={v.name}
                            label={v.name}
                            size={42}
                            fontFamily={`var(${v.cssVar})`}
                            weight={400}
                            text="PROSPECTOR / 1284"
                        />
                    ))}
                </SpecimenSection>

                {/* ========== PIXEL LINE — Stylistic sets ========== */}
                <SpecimenSection title="Geist Pixel Line" subtitle="All stylistic sets — ss01 to ss09">
                    {pixelLineSets.map((ss) => (
                        <SpecimenRow
                            key={`line-${ss}`}
                            label={ss}
                            size={42}
                            ...
                            weight={400}
                            text="PROSPECTOR / 1284"
                            fontFeatures={ss === 'default' ? undefined : `'${ss}' 1`}
                        />
                    ))}
                </SpecimenSection>

                {/* ========== ALL PIXEL VARIANTS × ss01 (comparison row) ========== */}
                <SpecimenSection title="All Pixel variants · ss01 activated" subtitle="Side-by-side comparison with stylistic set 1">
                    {pixelVariants.map((v) => (
                        <SpecimenRow
                            key={`${v.name}-ss01`}
                            label={`${v.name} · ss01`}
...
function SpecimenSection({ title, subtitle, children }: { title: string; subtitle: string; children: React.ReactNode }) {
    return (
        <div style={{ marginBottom: 64 }}>
            ...
            >
                <h3 className="font-sans" style={{ fontSize: 22, fontWeight: 500, letterSpacing: '-0.015em' }}>
                    {title}
                </h3>
                <span className="font-mono" style={{ fontSize: 11, opacity: 0.55, letterSpacing: '0.06em' }}>
                    {subtitle}
                </span>
            </div>
